"""Training orchestrator with TensorBoard logging.

Original: src/train_main.py (Minivision, zhuying)
"""
...
from app.ml.utils import get_time
from app.ml.models.multiftnet import MultiFTNet
...
class CheckpointManager:
    ...
    def save(self, model: torch.nn.Module, step: int, job_name: str) -> str:
        time_stamp = get_time()
        path = f'{self.save_dir}/{time_stamp}_{job_name}_model_iter-{step}.pth'
        torch.save(model.state_dict(), path)
        logger.info("checkpoint_saved", path=path, step=step)
        return path
...
class Trainer:
    def __init__(self, conf: TrainConfig):
        ...
        self.checkpoint = CheckpointManager(conf.model_path)

    def train(self) -> None:
        self._init_model_param()
        self._train_stage()

    def _init_model_param(self) -> None:
        ...
        logger.info("training_init", lr=self.conf.lr, epochs=self.conf.epochs,
    ...
    def _train_stage(self) -> None:
        ...
        for e in range(self.start_epoch, self.conf.epochs):
            if is_first:
                ...
                is_first = False
            logger.info("epoch_started", epoch=e, lr=self.schedule_lr.get_last_lr())
            ...
            for sample, ft_sample, target in tqdm(iter(self.train_loader)):
                ...
                if self.step % self.save_every == 0 and self.step != 0:
                    self.checkpoint.save(self.model, self.step, self.conf.job_name)
    ...
    def _train_batch_data(
        self, imgs: list[torch.Tensor], labels: torch.Tensor,
        ...
        loss = self.conf.cls_loss_weight * loss_cls + self.conf.ft_loss_weight * loss_fea
        acc = self._get_accuracy(embeddings, labels)[0]
        loss.backward()
        self.optimizer.step()
        return loss.item(), acc, loss_cls.item(), loss_fea.item()

    def _define_network(self) -> torch.nn.Module:
        param = {
