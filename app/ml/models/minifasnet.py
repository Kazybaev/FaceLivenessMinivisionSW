"""MiniFASNet architectures for face anti-spoofing.

CRITICAL: Do NOT change layer structure — existing .pth weights depend on it.

Original: src/model_lib/MiniFASNet.py (Minivision, zhuying)
"""
...
class ResidualSE(Module):
    def __init__(
        ...
        for i in range(num_block):
            ...
            if i == num_block - 1:
                modules.append(
                    Depth_Wise_SE(c1_tuple, c2_tuple, c3_tuple, residual=True, kernel=kernel,
                                  padding=padding, stride=stride, groups=groups, se_reduct=se_reduct))
            else:
                modules.append(Depth_Wise(c1_tuple, c2_tuple, c3_tuple, residual=True, kernel=kernel,
...
class MiniFASNetSE(MiniFASNet):
    def __init__(
        ...
        super(MiniFASNetSE, self).__init__(keep=keep, embedding_size=embedding_size,
                                           conv6_kernel=conv6_kernel, drop_p=drop_p,
...
# Channel configurations — DO NOT MODIFY (weights depend on these exact values)
keep_dict = {
